...
- Run State Foundation v1 has an initial Party Wipe path in C++: GameMode evaluates all current players dead, while GameState replicates `CombatActive` / `PartyWiped` state for clients and Blueprint presentation.
- Formal weapon/socket traces, final hit-reaction animation/content polish, dodge invulnerability, bow projectiles, final HUD/settings UI, revive/respawn, final death presentation polish, Encounter Clear, Portal, rewards/progression, and StateTree enemy behavior are not implemented yet.
