...
- Stage 5 combat feedback hardening has a validated minimal path: confirmed non-lethal enemy hits now have a server-triggered multicast presentation hook visible to both listen-server PIE windows, enemy hit/death presentation can use configured default Montages plus Blueprint extension hooks, friendly player hit reaction works without friendly damage, and the light attack path routes targets through a narrow combat target interface with separate hit-reaction and damage permission. Enemy Attack Path v1 is validated with the shared `LPQ Combat Hit Window` Notify: enemies can attack players through the server-authoritative GameplayAbility / GameplayEffect path while same-team hits remain no-damage presentation feedback. Enemy melee attacks now use a server-controlled telegraph plus a configurable foot-centered fan attack shape for readable range and Montage Notify-driven damage confirmation.
...
- Portal Event Foundation v1 is implemented in C++: `IA_Interact` is the unified interact input path for `E`, the player sends server-validated interact requests through a reusable interactable interface, and a placed portal can start `PortalEventActive` from player interaction instead of relying on encounter clear as the long-term activation gate. Boss Spawn On Portal v1 lets the Portal optionally spawn one existing enemy Blueprint as a test Boss; charging waits for that Boss to die, then still requires all living players in range before marking floor transition ready. Portal Charge Pressure v1 lets the Portal spawn ordinary pressure enemies during charging. Portal Destination Foundation v1 separates Portal completion mode from destination ID: Combat Event portals keep the Boss / charging / pressure loop, while Direct Floor Exit portals require all living players in the Portal radius and then go straight to the floor transition stub. Portal Branch Choice Lock v1 lets multiple Combat Event branch portals exist in one floor while the server locks the event to the interacted Portal, so only that Portal activates and its destination is used. Enemy Reward Ownership Cleanup v1 makes enemy death XP come directly from each enemy's own `ExperienceReward`; setting it to 0 makes a no-XP enemy.
- Floor Transition Stub v1 is validated: after portal readiness or direct floor exit, the server advances a replicated floor index, writes a replicated destination ID for debug/branch validation, resets placed portals, and returns the run state to combat without granting rewards or loading a new map.
- Enemy Spawn Source Foundation v1 is implemented in C++: ordinary floor waves now use placed `ALPQFloorWaveEnemySpawnSource` actors that own enemy class, count, spawn location, optional radius sampling, NavMesh projection, generated-enemy tracking, and cleanup. Hand-placed enemies no longer need registry membership for XP; all enemies use their own reward configuration. Portal Boss and pressure spawns remain Portal-owned fixed Actor transform references for now.
...
- LPQ naming cleanup has migrated current C++ reflected type names, C++ source filenames, Blueprint-facing API names, project-level Blueprint paths, and Config paths to `LPQ`; editor assets and labels can use shorter domain names when that is clearer. The old `Tunic` CoreRedirect compatibility layer has been retired.
