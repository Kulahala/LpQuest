...
- Run State Foundation v1 has initial Party Wipe and Encounter Clear paths in C++: GameMode evaluates all current players dead or all current enemies dead, while GameState replicates `CombatActive` / `PartyWiped` / `EncounterCleared` state for clients and Blueprint presentation.
- Formal weapon/socket traces, final hit-reaction animation/content polish, dodge invulnerability, bow projectiles, final HUD/settings UI, revive/respawn, final death presentation polish, Portal, rewards/progression, and StateTree enemy behavior are not implemented yet.
