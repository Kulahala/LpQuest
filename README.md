...
- Dodge / Roll v1.2 is implemented in C++: dodge input activates a server-only GameplayAbility with data-tuned cooldown/action-lock and `State.Invulnerable` durations, moves through server-authoritative manual RootMotionSource short dash, and plays the configured Dodge Montage as presentation with an adjustable play rate.
