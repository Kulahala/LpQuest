...
- Enemy AI v1 has the C++ StateTree chase/attack loop in place, and AI Perception / Patrol / Awareness support has been added: enemies use a server AIController with a StateTree AI brain, sight perception, configurable awareness policies, proximity aggro, delayed combat-spawn aggro, idle scan, placed patrol point routes, native StateTree nodes, and the existing enemy melee Ability request path. The prototype auto-attack switch is no longer the intended AI path.
