...
- Dodge / Roll v1.1 is implemented in C++: dodge input activates a server-only GameplayAbility with cooldown/action-lock tags, grants a short server-authoritative `State.Invulnerable` window, moves through server-authoritative manual RootMotionSource short dash, and can play a configured Dodge Montage as presentation.
