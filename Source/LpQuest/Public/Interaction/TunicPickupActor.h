...
protected:
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Tunic|Pickup")
	TObjectPtr<USceneComponent> SceneRoot;

	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Tunic|Pickup")
	TObjectPtr<UStaticMeshComponent> PickupMesh;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Pickup", meta = (ToolTip = "拾取后写入玩家 PlayerState 的当前装备标记。None 会拒绝拾取并输出 warning，避免假成功。"))
	FName PickupId = NAME_None;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Pickup", meta = (ToolTip = "拾取物显示名。v1 只给 Blueprint/UI 表现使用，不参与 gameplay 判定。"))
	FText PickupDisplayName;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Debug", meta = (ToolTip = "是否输出 pickup 交互、拒绝和成功日志。只用于验证，不影响 gameplay。"))
	bool bLogPickupInteraction = true;

	UFUNCTION(BlueprintNativeEvent, Category = "Tunic|Pickup", meta = (ToolTip = "服务器确认拾取成功后、Actor 销毁前触发的表现 hook。不要在这里改装备状态。"))
