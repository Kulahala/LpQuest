...
protected:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Tunic|Dodge", meta = (ClampMin = "0.0", Units = "s", ToolTip = "Dodge cooldown 和 action-lock 的持续时间，单位秒。Ability 使用这个值覆盖 cooldown GameplayEffect 的实际 duration，服务器仍确认最终状态。"))
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Tunic|Dodge|Prediction", meta = (ToolTip = "Dodge predicted RootMotion AbilityTask 的实例名。只用于调试 RootMotionSource，不是玩法数值。"))
	FName DodgeRootMotionTaskName = TEXT("TunicDodgePredictedMovement");
...
private:
	...
	FVector ResolveDodgeDirection(const FGameplayEventData* TriggerEventData, const ATunicPlayerCharacter* PlayerCharacter) const;
	bool ApplyPredictedDodgeMovement(ATunicPlayerCharacter* PlayerCharacter, const FVector& DodgeDirection);
};
