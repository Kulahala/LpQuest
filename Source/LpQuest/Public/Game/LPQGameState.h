...
#include "LPQGameState.generated.h"

class FLifetimeProperty;
class AActor;

UENUM(BlueprintType)
enum class ELPQRunState : uint8
{
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FLPQRunStateChangedSignature, ELPQRunState, NewRunState);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FLPQFloorIndexChangedSignature, int32, NewFloorIndex);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FLPQFloorDestinationChangedSignature, FName, NewFloorDestinationId);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FLPQSharedRunExperienceChangedSignature, int32, NewValue, int32, Delta, AActor*, SourceActor);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FLPQSharedRunLevelChangedSignature, int32, NewLevel);

UCLASS(Blueprintable)
class LPQUEST_API ALPQGameState : public AGameStateBase
{
	GENERATED_BODY()
...
public:
	...
	ELPQRunState GetRunState() const;
	...
	FLPQRunStateChangedSignature OnRunStateChangedEvent;

	UPROPERTY(BlueprintAssignable, Category = "LPQ|Run")
	FLPQFloorIndexChangedSignature OnFloorIndexChangedEvent;

	UPROPERTY(BlueprintAssignable, Category = "LPQ|Run")
	FLPQFloorDestinationChangedSignature OnFloorDestinationChangedEvent;

	UPROPERTY(BlueprintAssignable, Category = "LPQ|Run")
	FLPQSharedRunExperienceChangedSignature OnSharedRunExperienceChangedEvent;

	UPROPERTY(BlueprintAssignable, Category = "LPQ|Run")
	FLPQSharedRunLevelChangedSignature OnSharedRunLevelChangedEvent;

	void SetRunState(ELPQRunState NewRunState);
...
protected:
	...
	void OnRunStateChanged(ELPQRunState NewRunState);
...
private:
	...
	ELPQRunState RunState = ELPQRunState::CombatActive;
