...
public:
	...
	UFUNCTION(BlueprintPure, Category = "Tunic|Spawn", meta = (ToolTip = "返回本生成源当前仍存在的 generated 敌人数量，包含已死亡但尚未清理的尸体。"))
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Spawn", meta = (ClampMin = "0", ToolTip = "本生成源一次生成的敌人数量。0 表示不生成。"))
	int32 Count = 1;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Spawn", meta = (ClampMin = "0.0", Units = "cm", ToolTip = "随机生成半径。0 表示使用本 Actor 原始 Transform，不随机也不投射 NavMesh。"))
	float SpawnRadius = 0.0f;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Spawn", meta = (EditCondition = "SpawnRadius > 0.0", ToolTip = "SpawnRadius 大于 0 时，是否把随机点投射到 NavMesh。"))
	bool bProjectRandomSpawnToNavigation = true;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Spawn", meta = (EditCondition = "SpawnRadius > 0.0", ToolTip = "随机点投射到 NavMesh 时使用的搜索范围。"))
	FVector NavProjectionExtent = FVector(300.0f, 300.0f, 500.0f);

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tunic|Debug", meta = (ToolTip = "是否输出生成源刷怪、NavMesh 投射失败和清理日志。只用于验证，不影响玩法。"))
...
public:
	UFUNCTION(BlueprintPure, Category = "Tunic|Spawn", meta = (ToolTip = "返回本生成源最近一次生成对应的 floor index。0 表示尚未生成。"))
