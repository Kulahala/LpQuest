...
ALPQEnemyPatrolRoute::ALPQEnemyPatrolRoute()
{
...
void ALPQEnemyPatrolRoute::OnConstruction(const FTransform& Transform)
{
...
void ALPQEnemyPatrolRoute::BeginPlay()
{
	Super::BeginPlay();
...
void ALPQEnemyPatrolRoute::Tick(float DeltaSeconds)
{
...
FName ALPQEnemyPatrolRoute::GetRouteId() const
{
	return RouteId;
}

int32 ALPQEnemyPatrolRoute::GetPatrolPointCount() const
{
	return SampledPatrolLocations.Num();
}

FVector ALPQEnemyPatrolRoute::GetPatrolPointLocation(int32 PointIndex) const
{
...
bool ALPQEnemyPatrolRoute::IsPatrolPointStop(int32 PointIndex) const
{
	const int32 PointCount = SampledPatrolLocations.Num();
	if (PointCount <= 0)
	{
...
float ALPQEnemyPatrolRoute::GetPatrolStopHoldDuration(int32 PointIndex) const
{
	return IsPatrolPointStop(PointIndex) ? FMath::Max(0.0f, PatrolStopHoldDuration) : 0.0f;
}

bool ALPQEnemyPatrolRoute::IsLoopRoute() const
{
	return bLoopRoute;
}

USplineComponent* ALPQEnemyPatrolRoute::GetPatrolSplineComponent() const
{
	return PatrolSplineComponent;
}

void ALPQEnemyPatrolRoute::RebuildSampledPatrolLocations()
{
...
void ALPQEnemyPatrolRoute::AddSampledPatrolLocation(const FVector& RawLocation, bool bIsStop)
{
	...
	FLPQPatrolRouteSample NewSample;
	NewSample.Location = ProjectedLocation;
	NewSample.bIsStop = bIsStop;
	SampledPatrolLocations.Add(NewSample);
}

int32 ALPQEnemyPatrolRoute::FindNearestSampleIndexToDistance(float DistanceAlongSpline) const
{
...
FVector ALPQEnemyPatrolRoute::ProjectLocationToNavigation(const FVector& RawLocation) const
{
...
void ALPQEnemyPatrolRoute::DrawRuntimeDebugRoute() const
{
