...
When strict review finds a non-blocking risk, triage it instead of leaving it only in chat: fix blockers immediately, keep current-stage accepted risks in `plan.md`, and put durable future-stage risks in `tunicplan.md` under the stage or Recommendation that will expose them. Use a short `Warning:` line plus the trigger/decision needed at that future stage; do not turn `tunicplan.md` into a review log.
