...
Current dodge input activates a minimal `UGameplayAbility` tagged `Ability.Movement.Dodge` through the player ASC. The ability executes on the server, applies a native short cooldown/action-lock GameplayEffect with `Cooldown.Dodge` and `State.ActionLocked`, then delegates to the player character's dodge execution path. Dodge movement is a server-authoritative short `FRootMotionSource_ConstantForce` on the character movement component, using recent movement input direction and falling back to ActorForward after a short input grace window. The configured Dodge Montage is presentation only and is multicast by the server. Dodge v1.1 does not grant invulnerability, consume Stamina, apply gameplay rewards, or rely on animation Root Motion.
...
`UTunicRunStatusWidget` is the current minimal HUD surface for run-loop validation. It is created locally by `ATunicPlayerController`, binds to `ATunicGameState` and the owning `ATunicPlayerState` presentation events, displays floor index, run state, shared run XP, shared run level, and the local player's pending upgrade choices, and exposes a temporary button that requests the server selection stub through the owning PlayerController. It does not directly mutate XP, run state, floor index, level, pending choices, rewards, or combat state.
...
`UTunicGameplayAbility_Dodge` is the first minimal player movement GameplayAbility. It is server-only, granted by `ATunicPlayerCharacter` during player ASC initialization, uses a native dodge cooldown/action-lock GameplayEffect, blocks activation while `Cooldown.Dodge`, `State.ActionLocked`, or `State.Dead` is present, and delegates manual dodge movement plus Montage presentation to the player character. The ability does not mutate attributes, grant invulnerability, or own final movement tuning.
...
`ATunicEnemyAIController` is the first enemy AI controller. It owns a `UStateTreeAIComponent` brain that is started only on the server when a valid enemy pawn and StateTree asset are present. It also owns `UAIPerceptionComponent` sight awareness, the configured enemy awareness policy, current target storage, proximity aggro checks, combat-spawn delayed aggro, idle scan rotation, aggro forget timing, patrol-route state, range checks, focus, stopping movement, and requesting the current target melee attack. Target acquisition filters valid player targets through `ITunicCombatTargetInterface::IsCombatTargetAvailable()`. StateTree assets should consume the AI-controller-owned current target rather than running their own nearest-player scan task.

`ATunicEnemyPatrolRoute` is the placed patrol authoring actor. It owns a `USplineComponent` so designers can draw a visible route in the level, while AI patrol target locations are generated automatically by distance sampling along that spline. Enemy AI controllers bind to the route through `PatrolRouteId`, read the current sampled patrol location, and advance the route index. Runtime patrol target locations are projected to navigation when possible so accidentally low or high spline point Z values do not send AI below or above the walkable surface. The route actor can optionally draw the route curve and sampled patrol points during PIE validation. Patrol routes do not own encounter membership, rewards, spawn logic, combat results, or replicated gameplay state.

`ST_EnemyMeleeBasic` uses `Patrol` as the no-current-target parent state. That parent state should remain an intent router rather than owning route movement tasks directly. Its children are `PatrolRoute`, which is entered when the AI controller has a patrol route and consumes the current patrol location, and `IdleAtHome`, which is entered when no patrol route is available. `Chase` consumes the AI-controller-owned current target and moves to that actor. `Attack` requests the existing enemy melee GameplayAbility and does not own hit confirmation or damage. StateTree states that combine instant Tunic tasks with `Move To` or `Delay` use Tasks completion mode `All` so instant tasks do not prematurely complete movement or recovery states.

StateTree decides intent: idle, patrol, alert, chase, combat, attack, recover, hit react, dead. GameplayAbilities execute concrete actions. Native Tunic StateTree nodes may read the current target selected by the AI controller, read patrol locations, advance patrol targets, check attack range through the AI controller, and ask the enemy AI controller to request melee attack activation. They must not directly mutate Health, RunState, XP, GameplayTags, or final combat state. The current prototype auto attack on enemies is only a debug validation trigger for the enemy melee path and is not the main AI path.
