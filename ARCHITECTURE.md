...
`ATunicGameMode` owns the current server-authoritative run-state decision point. Party Wipe v1 is evaluated on the server by scanning the current `GameState->PlayerArray` for controlled `ATunicPlayerCharacter` pawns and checking their replicated character death state. Encounter Clear v1 is evaluated on the server by scanning active-level `ATunicEnemyCharacter` actors and checking enemy death state; after floor advancement it requires seeing at least one living enemy on the current floor before another clear can trigger, which prevents old dead enemies from immediately clearing a placeholder floor. Portal v1 completes through a GameMode-owned `EncounterCleared -> FloorTransitionReady` transition. Floor Transition Stub v1 then delays briefly, advances the replicated floor index, resets placed portals, and returns the run state to `CombatActive`. Characters expose their own death state, but they do not decide whole-party failure, encounter completion, floor-transition readiness, or floor advancement.

`ATunicGameState` owns the replicated run state surface through `ETunicRunState` and the current run-local floor index. The first implemented states are `CombatActive`, `PartyWiped`, `EncounterCleared`, and `FloorTransitionReady`. `ATunicGameState` replicates the state and floor index to clients and exposes Blueprint presentation hooks for state and floor changes; it does not decide when the party is wiped, the encounter is cleared, the portal is ready, or the floor advances.
