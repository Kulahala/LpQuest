...
`ATunicPickupActor` is the first minimal world pickup implementation. It implements `ITunicInteractableInterface`, exposes a `PickupId` and display name for authored Blueprint setup, and uses the same server-validated `E` interaction path as the Portal. On a valid server interaction from a living player, it writes `PickupId` to that player's `ATunicPlayerState::CurrentEquipmentId` and destroys itself. `PickupId=None` rejects interaction with a warning instead of pretending the pickup succeeded. The pickup actor does not grant abilities, modify attributes, own inventory, change attack data, or route drops.
