...
`ATunicEnemyAIController` is the first enemy AI controller. It owns a `UStateTreeAIComponent` brain that is started only on the server when a valid enemy pawn and StateTree asset are present. It also owns `UAIPerceptionComponent` sight awareness, the configured enemy awareness policy, current target storage, proximity aggro checks, combat-spawn delayed aggro, idle scan rotation, aggro forget timing, patrol-route state, range checks, focus, stopping movement, and requesting the current target melee attack. Target acquisition filters valid player targets through `ITunicCombatTargetInterface::IsCombatTargetAvailable()`. The older nearest-target StateTree task remains as a compatibility helper, but it now obeys the configured awareness policy instead of doing unconditional all-player scanning.

`ATunicEnemyPatrolPoint` is the current lightweight placed patrol authoring actor. Enemy AI controllers can collect points by `RouteId`, sort them by `OrderIndex`, and expose the current patrol target to StateTree. Patrol points do not own encounter membership, rewards, spawn logic, combat results, or replicated gameplay state.

StateTree decides intent: idle, patrol, alert, chase, combat, attack, recover, hit react, dead. GameplayAbilities execute concrete actions. Native Tunic StateTree nodes may read the current target selected by the AI controller, read/advance patrol targets, check attack range through the AI controller, and ask the enemy AI controller to request melee attack activation. They must not directly mutate Health, RunState, XP, GameplayTags, or final combat state. The current prototype auto attack on enemies is only a debug validation trigger for the enemy melee path and is not the main AI path.
