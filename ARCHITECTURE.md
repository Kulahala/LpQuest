...
`ATunicEnemyCharacter` owns its own ASC and AttributeSet because enemies do not have PlayerStates. It initializes GAS with itself as OwnerActor and AvatarActor. It currently has a minimal replicated death state: when server Health reaches 0, the enemy replicates `bIsDead`, disables capsule collision and character movement, calls a Blueprint death-state hook, and adds the registered `State.Dead` loose gameplay tag. Non-lethal confirmed hits can call a multicast hit-reaction hook; this is presentation-only and does not own damage, stun, knockback, or AI state changes.
