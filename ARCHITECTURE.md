...
`UTunicRunStatusWidget` is the current minimal HUD surface for run-loop validation. It is created locally by `ATunicPlayerController`, binds to `ATunicGameState` presentation events, and displays floor index, run state, shared run XP, and shared run level. It does not mutate XP, run state, floor index, level, rewards, or combat state.
