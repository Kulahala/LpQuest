...
Current dodge input activates a minimal `UGameplayAbility` tagged `Ability.Movement.Dodge` through the player ASC. The ability executes on the server, applies a native short cooldown/action-lock GameplayEffect with `Cooldown.Dodge` and `State.ActionLocked`, applies a separate short invulnerability GameplayEffect that grants `State.Invulnerable`, then delegates to the player character's dodge execution path. Dodge movement is a server-authoritative short `FRootMotionSource_ConstantForce` on the character movement component, using recent movement input direction and falling back to ActorForward after a short input grace window. The configured Dodge Montage is presentation only and is multicast by the server. Dodge v1.1 does not consume Stamina, apply gameplay rewards, or rely on animation Root Motion.
