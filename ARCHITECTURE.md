...
`ATunicEnemyAIController` is the first enemy AI controller. It owns a `UStateTreeAIComponent` brain that is started only on the server when a valid enemy pawn and StateTree asset are present. It also owns sight-only `UAIPerceptionComponent` target awareness, current target storage, aggro forget timing, patrol-route state, range checks, focus, stopping movement, and requesting the current target melee attack. Target acquisition now prefers perceived valid player targets and filters them through `ITunicCombatTargetInterface::IsCombatTargetAvailable()`; the older player-array scan remains only as a compatibility helper for bootstrap StateTree assets.
