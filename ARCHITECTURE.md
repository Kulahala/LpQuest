...
Current dodge input activates a minimal `UGameplayAbility` tagged `Ability.Movement.Dodge` through the player ASC. The ability executes on the server, applies a native cooldown/action-lock GameplayEffect with `Cooldown.Dodge` and `State.ActionLocked`, applies a separate invulnerability GameplayEffect that grants `State.Invulnerable`, then delegates to the player character's dodge execution path. The dodge Ability owns the gameplay timing values for cooldown/action-lock and invulnerability by applying GameplayEffect specs with explicit durations. Dodge movement is a server-authoritative short `FRootMotionSource_ConstantForce` on the character movement component, using recent movement input direction and falling back to ActorForward after a short input grace window. The player character owns movement tuning and Dodge Montage presentation tuning, including montage play rate. The configured Dodge Montage is presentation only and is multicast by the server; it does not decide movement distance, invulnerability timing, or final combat results. Dodge v1.2 does not consume Stamina, apply gameplay rewards, rely on animation Root Motion, or use Montage NotifyState windows for invulnerability.
...
`UTunicGameplayAbility_Dodge` is the first minimal player movement GameplayAbility. It is server-only, granted by `ATunicPlayerCharacter` during player ASC initialization, uses a native dodge cooldown/action-lock GameplayEffect, applies a separate native dodge invulnerability GameplayEffect, blocks activation while `Cooldown.Dodge`, `State.ActionLocked`, or `State.Dead` is present, and delegates manual dodge movement plus Montage presentation to the player character. The ability owns cooldown/action-lock and invulnerability durations as editable gameplay timing values; the player character owns distance, movement duration, input-direction grace, root-motion priority, and Montage play rate. The ability does not mutate attributes or own final movement displacement tuning.
